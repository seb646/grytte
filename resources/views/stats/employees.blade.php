...
<div class="bg-gray-50 py-15">
    <div class="container mx-auto px-5 sm:px-40">
                ...
                <div>
                    <div class="block">
                      <nav class="smLflex">
                        ...
                        <a href="/stats/employees" class="block sm:inline-block px-3 py-2 font-medium text-sm leading-5 rounded-md text-gray-700 bg-gray-200 hover:text-gray-700 focus:outline-none focus:text-gray-700 focus:bg-gray-100">
        ...
        @if(count($years) >= 2)
        @if($name != NULL)
        <div id="{{str_replace('\'', '', str_replace(' ', '_', str_replace('.', '', strtolower($name))))}}" class="bg-white shadow overflow-hidden rounded-lg mt-12">
            <div class="px-4 py-5 border-b border-gray-200 sm:px-6 bg-gray-600">
                <div class="-ml-4 -mt-4 flex justify-between items-center flex-wrap sm:flex-no-wrap">
                    <div class="ml-4 mt-4">
                      <a href="#{{str_replace('\'', '', str_replace(' ', '_', str_replace('.', '', strtolower($name))))}}" class="text-lg leading-6 font-medium text-white">
                        {{$name}}
                      </a>
                    </div>
                </div>
            ...
            <div class="px-4 py-5 sm:px-6">
                <div class="w-full">
                    <canvas id="chart_{{str_replace('\'', '', str_replace(' ', '_', str_replace('.', '', strtolower($name))))}}"></canvas>
                </div>
            </div>
        </div>
        @endif
...
@if($name != NULL)
<script>
var ctx = document.getElementById(<?php echo '\'chart_'.str_replace('\'', '', str_replace(' ', '_', str_replace('.', '', strtolower($name)))).'\'';?>);
var myChart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: [<?php foreach($years as $key => $value){ echo $key.',';}?>],
        datasets: [{
            label: 'USD',
            data: [<?php foreach($years as $key => $value){ echo $value.',';}?>],
            backgroundColor: [
                'rgba(104, 211, 145, .2)',
            ],
            borderColor: [
